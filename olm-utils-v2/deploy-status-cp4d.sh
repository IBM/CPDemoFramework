...
API_RESOURCES=$(oc api-resources --namespaced=true --verbs=list -o name 2>/dev/null | grep -E 'ibm|manta' | awk '{printf "%s%s",sep,$0;sep=","}')
if [ ! -z ${API_RESOURCES} ];then
    log "Getting Custom Resources in OpenShift project ${CP4D_PROJECT}..."
    oc get --no-headers -n $CP4D_PROJECT ${API_RESOURCES}  --ignore-not-found -o=custom-columns=KIND:.kind,NAME:.metadata.name --sort-by='kind' 2>/dev/null > ${temp_dir}/cp4d-resources.out
    while read -r line;do
        ...
        case $CR in
            ...
            ;;
            *)
            cr_status=$(oc get -n $CP4D_PROJECT $CR $CR_NAME -o jsonpath='{.status}' 2>/dev/null | jq -r '. | to_entries | map(select(.key | match("Status";"i"))) | map(.value) | first')
