...
if [ ! -z ${API_RESOURCES} ];then
    ...
    oc get --no-headers -n $CP4I_PROJECT ${API_RESOURCES}  --ignore-not-found -o=custom-columns=KIND:.kind,NAME:.metadata.name --sort-by='kind' > ${temp_dir}/cp4i-resources.out
    while read -r line;do
        ...
        case $CR in
            ...
            ;;
            *)
            cr_status=$(oc get -n $CP4I_PROJECT $CR $CR_NAME -o jsonpath='{.status}' 2>/dev/null | jq -r '. | to_entries | map(select(.key | match("Status"))) | map(.value) | first')
            if [[ "${cr_status}" != "" ]] && [[ ${cr_status} != "null" ]];then
                echo "${CR} - ${CR_NAME} - ${cr_status}"
            fi
            ;;
        esac
    done < ${temp_dir}/cp4i-resources.out
fi 
...
cp4i_admin_password=$(oc get secrets -n ibm-common-services platform-auth-idp-credentials -ojsonpath='{.data.admin_password}' | base64 -d && echo "" 2>/dev/null)
if [ "${cp4i_host}" != "" ];then
