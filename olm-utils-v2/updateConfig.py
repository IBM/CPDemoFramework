...
#update opneshift configuration yaml
with open('openshift-config.yaml') as f:
    list_ocs = yaml.safe_load(f)

if storage_vendor.lower()=='nfs':
    list_ocs["openshift"][0]["openshift_storage"][0]["storage_name"]= "nfs-storage"
    list_ocs["openshift"][0]["openshift_storage"][0]["storage_type"]= "nfs"

elif storage_vendor.lower()=='ocs':
    list_ocs["openshift"][0]["openshift_storage"][0]["storage_name"]= "ocs-storage"
    list_ocs["openshift"][0]["openshift_storage"][0]["storage_type"]= "ocs"

elif storage_vendor.lower()=='portworx':
    list_ocs["openshift"][0]["openshift_storage"][0]["storage_name"]= "portworx-storage"
    list_ocs["openshift"][0]["openshift_storage"][0]["storage_type"]= "portworx"
    ...
    list_ocs["openshift"][0]["openshift_storage"][0]["ocp_storage_class_block"]= "ibmc-block-gold"
    list_ocs["openshift"][0]["openshift_storage"][0]["ocp_storage_class_file"] = "ibmc-file-gold-gid"

    with open("openshift-config.yaml", "w") as f:
        yaml.dump(list_ocs, f, sort_keys=False)
...
if(cpak.lower()=='cp4d'):
    list_cp4i["cp4d"][0]["cp4d_version"] = version
