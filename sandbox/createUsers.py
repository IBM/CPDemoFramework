...
for i in range(len(userTableDeafult)):
    usersTable.loc[len(usersTable.index)] = [userTableDeafult.loc[i,'username'],CPD_USER_PASSWORD if userTableDeafult.loc[i,'username']==CPD_USER_NAME else password,userTableDeafult.loc[i,'email'],userTableDeafult.loc[i,'displayName'],userTableDeafult.loc[i,'user_roles']] 
